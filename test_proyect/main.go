...
func actualizarPantalla(t tcell.Screen) {
	...
	dibujarTabla(t, 1, 9, ancho/2-1, alto-10, "+ RAM", ordenarPorRAM(procesos))
	dibujarTabla(t, ancho/2+1, 9, ancho/2-2, alto-10, "+ CPU", ordenarPorCPU(procesos))
	t.Show()
}
...
func dibujarTabla(t tcell.Screen, x, y, ancho, alto int, titulo string, lista []Proceso) {
	...
	for i, p := range lista {
		...
		dibujarTexto(t, x, y+2+i, texto, tcell.StyleDefault)
	}
}
